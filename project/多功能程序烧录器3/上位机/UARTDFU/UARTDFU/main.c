...
static DWORD CALLBACK download_firmware_process(LPVOID lpParameter)
{
	...
	//timeouts.WriteTotalTimeoutMultiplier = 50; // 在写入时，每字符可等待的总超时时间的部分
	timeouts.WriteTotalTimeoutConstant = 10;

	//timeouts.ReadIntervalTimeout = 1000; // 在读取时，如果没有字符可读，等待1000ms后超时
    //timeouts.ReadTotalTimeoutMultiplier = 1000; // 在读取时，每字符可等待的总超时时间的部分
    //timeouts.ReadTotalTimeoutConstant = 1000; // 读取的固定超时时间
    //timeouts.WriteTotalTimeoutMultiplier = 1000; // 在写入时，每字符可等待的总超时时间的部分
    //timeouts.WriteTotalTimeoutConstant = 1000; // 写入的固定超时时间
	...
	if (!downloading)
		goto end;
SetDlgItemText(hwndDlg, IDC_STATIC5, TEXT("请按下复位键22"));
	set_progress(0);
	...
	case HEADER_FIRMWARE_INFO:
		...
		while (downloading)
		{
			...
			for (j = 0; j< resp.size; j++) {
				write_port(hPort, hexfile.data + (resp.addr - hexfile.start_addr) + j, 1);
			}
